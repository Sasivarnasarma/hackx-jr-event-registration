...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (session.user.role !== "SUPER_ADMIN") {
      ...
      return sendError(
        "FORBIDDEN",
        "Only Super Admins can manage administrator requests.",
        requestId,
        403
      );
    }
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (session.user.role !== "SUPER_ADMIN") {
      return sendError(
        "FORBIDDEN",
        "Only Super Admins can manage administrator approvals.",
        requestId,
        403
      );
    }
    ...
    if (userId === undefined || userId === null || !status) {
    ...
    if (targetUser.role === "SUPER_ADMIN") {
      return sendError(
        "FORBIDDEN",
        "Super Admins cannot modify status parameters of other Super Admins.",
        requestId,
        403
      );
    }
    ...
    logger.info(
      { requestId, superAdminId: session.userId, targetId, newStatus: status },
    ...
    if (status !== "APPROVED") {
      ...
      logger.info({ requestId, targetId }, "Revoked all active sessions for non-approved user");
    }

    return sendSuccess(
      updatedUser,
      `Administrator request status updated to ${status}.`,
      requestId
    );
  } catch (error: any) {
    logger.error(
      { requestId, error: error.message },
      "Error updating administrator request status"
    );
    return sendError("INTERNAL_SERVER_ERROR", "An unexpected error occurred.", requestId, 500);
  }
}
