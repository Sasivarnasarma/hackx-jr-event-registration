...
import { registrationSchema, normalizeMobileNumber, normalizeEmail, normalizeSpaces } from "@/lib/validation";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!isHuman) {
      ...
      return sendError("TURNSTILE_ERROR", "Bot verification failed. Please try again.", requestId, 400);
    }
    ...
    const normalizedSource = data.awarenessSource ? normalizeSpaces(data.awarenessSource) : "Not Specified";
    ...
    if (duplicateMobile) {
      logger.warn({ requestId, mobileNumber: normalizedMobile }, "Duplicate mobile number registration attempt");
      return sendError(
        "DUPLICATE_MOBILE",
    ...
    logger.error({ requestId, error: error.message, stack: error.stack }, "Unhandled error during registration");
